defmodule Explorer.Chain.AdvancedFilter do
  ...
  defp filter_transactions_by_addresses(query, from_addresses, to_addresses, relation, order_by) do
    ...
    case {process_address_inclusion(from_addresses), process_address_inclusion(to_addresses)} do
      {nil, nil} -> query
      {from, nil} -> do_filter_internal_transactions_by_address(query, from, :from_address_hash, order_by)
      {nil, to} -> do_filter_internal_transactions_by_address(query, to, :to_address_hash, order_by)
      {from, to} -> do_filter_internal_transactions_by_both_addresses(query, from, to, relation, order_by)
    end
  end

  defp do_filter_internal_transactions_by_address(query, {:include, addresses}, field, order_by) do
    queries =
      addresses
      |> Enum.map(fn address ->
        query
    ...
    filtered_query = from(internal_transaction in subquery(queries))
    filtered_query |> order_by.()
  end

  defp do_filter_internal_transactions_by_address(query, {:exclude, addresses}, field, order_by) do
    query
    |> where([internal_transaction], field(internal_transaction, ^field) not in ^addresses)
    |> order_by.()
  end

  defp do_filter_internal_transactions_by_both_addresses(query, {:include, from}, {:include, to}, relation, order_by) do
    from_queries =
      from
      |> Enum.map(fn from_address ->
        query
    ...
    do_filter_internal_transactions_by_both_addresses_to_include(from_queries, to_queries, relation, order_by)
  end
  ...
  defp do_filter_internal_transactions_by_both_addresses(query, {:include, from}, {:exclude, to}, :and, order_by) do
    ...
    filtered_query = from(internal_transaction in subquery(from_queries))
    filtered_query |> order_by.()
  end
  ...
  defp do_filter_internal_transactions_by_both_addresses(query, {:exclude, from}, {:include, to}, :and, order_by) do
    ...
    filtered_query = from(internal_transaction in subquery(to_queries))
    filtered_query |> order_by.()
  end
  ...
  defp do_filter_internal_transactions_by_both_addresses_to_include(from_queries, to_queries, relation, order_by) do
    case relation do
      :and ->
        united_from_queries =
          from_queries |> map_first(&subquery/1) |> Enum.reduce(fn query, acc -> union_all(acc, ^query) end)
        ...
        filtered_query = from(internal_transaction in subquery(intersect_all(united_from_queries, ^united_to_queries)))
      ...
      _ ->
        ...
        filtered_query = from(internal_transaction in subquery(union_query))
        filtered_query |> order_by.()
    end
  end
